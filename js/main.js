...
(function ($){
    ...
    $('#contact-form').on('submit', function (event) {
        ...
        $.ajax({
            url: "https://docs.google.com/forms/d/12g8YlEgHcJI2QPZ8vOjWZqVCJLIQkgoAycWEXB1g3JE/formResponse",
            data: { "entry.1124595091": name, 
            "entry.1158383789": email, 
            "entry.1456127285": subject, 
            "entry.1790425560": message },
            type: "POST",
            dataType: "xml",
        }).always(function () {
                    window.location.replace("contact_thankyou.html");
	            });
    });
    ...
    $('.filter-color-box').on('click', function (event) {
    	event.preventDefault()
        var c = $(this).data("color");
        var activeSibling = $(this).closest("ul").find("a.active")
